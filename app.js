...
function score(a,b){if(a.cat===b.cat)return -99;let s=0;if(a.color===b.color)s-=2;if(neutralColors.includes(a.color)||neutralColors.includes(b.color))s+=2;if(a.color==='하늘색'&&['네이비','흰색','회색','베이지'].includes(b.color))s+=4;if(a.color==='네이비'&&['흰색','회색','베이지','하늘색'].includes(b.color))s+=4;if(a.cat==='상의'&&b.cat==='하의')s+=2;if(a.cat==='아우터'&&(b.cat==='상의'||b.cat==='하의'))s+=1;return s;}
function weatherLookPenalty(c,w){let p=0,t=w?.apparent??w?.temp??20, rain=(w?.precipProbability??0)>=50 || (w?.precipitation??0)>0;if(c.cat==='아우터'){if(t>=24)p+=6;if(t<=14)p-=4;if(rain)p-=1}if(c.cat==='신발'&&rain)p+=2;if(c.season&&c.season!=='전체'){const s=autoSeasonFromDate();if(c.season!==s)p+=2}return p;}
function makeLooks(clothes,opts={}){const tops=clothes.filter(x=>x.cat==='상의'),bottoms=clothes.filter(x=>x.cat==='하의'),outs=opts.avoidOuter?[]:clothes.filter(x=>x.cat==='아우터'),shoes=clothes.filter(x=>x.cat==='신발');if(!tops.length||!bottoms.length)return[];let arr=[];tops.forEach(t=>bottoms.forEach(b=>{const o=outs.length?outs.reduce((p,x)=>score(t,x)+score(b,x)-weatherLookPenalty(x,todayWeather)>score(t,p)+score(b,p)-weatherLookPenalty(p,todayWeather)?x:p,outs[0]):null;const sh=shoes.length?shoes.slice().sort((a,b)=>weatherLookPenalty(a,todayWeather)-weatherLookPenalty(b,todayWeather))[0]:null;const ids=[t.id,b.id];if(o)ids.push(o.id);if(sh)ids.push(sh.id);arr.push({ids,score:score(t,b)+(o?score(t,o)+score(b,o)-weatherLookPenalty(o,todayWeather):0)});}));return arr.sort((a,b)=>b.score-a.score).slice(0,3);}
function recommend(){
  if(!state.clothes.length){alert('먼저 옷을 등록해주세요.');return}
  let pool=homeSeason==='전체'?state.clothes:state.clothes.filter(c=>c.season===homeSeason||c.season==='전체');let usedFallback=false;if(!pool.some(c=>c.cat==='상의')||!pool.some(c=>c.cat==='하의')){pool=state.clothes;usedFallback=true}
  const t=todayWeather.apparent??todayWeather.temp;const avoidOuter=t!=null&&t>=25&&!(todayWeather.precipProbability>=50);const looks=makeLooks(pool,{avoidOuter});if(!looks.length){alert('상의와 하의를 각각 1벌 이상 등록하면 코디를 추천할 수 있습니다.');return}
  const box=document.createElement('div');box.className='modal';const fallbackHtml=usedFallback?'<div class="ai-hint">'+homeSeason+' 옷이 부족해서 전체 옷장에서 추천했어요.</div>':'';
  const weatherHtml=todayWeather.temp?'<div class="weather-summary">'+weatherDisplay(todayWeather)+' '+todayWeather.temp+'℃ · 체감 '+(todayWeather.apparent??todayWeather.temp)+'℃ · 강수확률 '+(todayWeather.precipProbability??'-')+'%</div>':'';
  const looksHtml=looks.map((o,i)=>{const pieces=o.ids.map(id=>{const c=state.clothes.find(x=>x.id===id);return c?'<div class="piece"><img src="'+imgSrc(c)+'"></div>':''}).join('');const names=o.ids.map(id=>{const c=state.clothes.find(x=>x.id===id);return escapeHtml(c.name||c.cat)}).join(' · ');return '<div class="look"><b>추천 '+(i+1)+'</b><div class="look-grid">'+pieces+'</div><div class="meta">'+names+'</div><button class="secondary" style="margin-top:10px;width:100%" data-ids=\''+JSON.stringify(o.ids)+'\'>♡ 이 코디 저장</button></div>'}).join('');
  box.innerHTML='<div class="sheet"><h2>오늘의 코디</h2>'+weatherHtml+fallbackHtml+looksHtml+'<button class="secondary" style="width:100%" id="closeRecommend">닫기</button></div>';document.body.appendChild(box);box.querySelector('#closeRecommend').onclick=()=>box.remove();box.querySelectorAll('[data-ids]').forEach(btn=>btn.onclick=()=>saveOutfitFromBtn(btn,box));
}
async function saveOutfitFromBtn(btn,box){const ids=JSON.parse(btn.dataset.ids);btn.textContent='저장 중…';btn.disabled=true;try{await Storage.addOutfit({clothIds:ids,date:new Date().toLocaleDateString('ko-KR'),title:'오늘의 코디',weather:{temp:todayWeather.temp,condition:todayWeather.condition}});btn.textContent='저장됨 ✓';await reloadState();setTimeout(()=>{box.remove();tab='outfit';render()},400)}catch(e){alert('코디를 저장하지 못했어요.');btn.disabled=false;btn.textContent='♡ 이 코디 저장'}}
